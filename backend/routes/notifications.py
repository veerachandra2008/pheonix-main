...
def get_notifications():
    ...
    if user_email:
        filtered = [n for n in IN_MEMORY_NOTIFICATIONS if not n.get('user_email') or (n.get('user_email') or '').lower() == user_email]
        return jsonify({'success': True, 'data': filtered}), 200
...
def create_notification():
    ...
    try:
        supabase = get_supabase_client()
        # Clean payload to avoid inserting non-existent columns into Supabase
        clean_payload = {k: v for k, v in data.items() if k != 'time'}
        res = supabase.table('notifications').insert(clean_payload).execute()
