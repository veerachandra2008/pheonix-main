...
const Ferrofluid: React.FC<FerrofluidProps> = ({
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({
      // Cap DPR at 1.5 — on Retina/high-DPI rendering at native DPR is 2-3x GPU cost
      dpr: Math.min(dpr ?? (typeof window !== 'undefined' ? window.devicePixelRatio || 1 : 1), 1.5),
      alpha: true,
      antialias: false, // antialias is very expensive on mobile; remove for perf
    });
    ...
    const onPointerMove = (e: MouseEvent | PointerEvent) => {
      if (mousePendingRef.current) return; // throttle with rAF
    ...
    const loop = (t: number) => {
      ...
      // Skip rendering when element is not visible (e.g. footer below fold)
    ...
    // Pause render loop when not visible — saves GPU when footer is off-screen
